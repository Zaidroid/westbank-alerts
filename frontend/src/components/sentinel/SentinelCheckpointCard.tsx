...
export function SentinelCheckpointCard({ checkpoint, updates = [], onClick }: SentinelCheckpointCardProps) {
  ...
  const colorClass = STATUS_COLORS[checkpoint.status] || "bg-outline";
  const textColorClass = STATUS_TEXT_COLORS[checkpoint.status] || "text-on-surface";
  const pillClass = STATUS_PILL_CLASSES[checkpoint.status] || "bg-surface-variant border-outline-variant/30";
  const glowClass = STATUS_GLOW[checkpoint.status] || "";
      ...
      <div className="relative overflow-hidden rounded-[20px] bg-surface-container-low/70 backdrop-blur-md border border-outline-variant/20 hover:bg-surface-container hover:border-outline-variant/40 transition-all group flex flex-col shadow-sm">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center min-w-[70px] shrink-0 bg-surface-container-lowest/30 border-e border-outline-variant/10 py-4 gap-1">
            <span className="text-3xl font-headline font-bold text-on-surface leading-none tracking-tighter tabular-nums">
              {bigNumberDisplay}
            </span>
            <span className="text-[9px] font-label text-on-surface-variant font-bold uppercase tracking-widest text-center">
              {lang === 'ar' ? 'دقيقة' : 'MINS AGO'}
            </span>
          </div>
          ...
          <div className="flex-1 min-w-0 p-4 pb-3 flex flex-col justify-center">
            ...
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              <div className={cn("flex items-center gap-1.5 px-2 py-0.5 rounded-md border", pillClass)}>
                <div className={cn("w-1.5 h-1.5 rounded-full", colorClass, glowClass)} />
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-label text-on-surface-variant mt-0.5">
              ...
              {formattedTime}
              {updates.length > 0 && (
                <span className="text-on-surface-variant/50">• {updates.length} {lang === 'ar' ? 'تحديث' : 'updates'}</span>
              )}
            </div>
          </div>
            ...
            <button
              className={cn(
                ...
                expanded
                  ...
                  : "bg-surface-container-high group-hover:bg-primary group-hover:text-primary-container text-on-surface-variant"
              )}
                ...
                <div className="w-full px-5 flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className={cn("w-2 h-2 rounded-full", colorClass, glowClass)} />
                      <span className={cn("text-[11px] font-bold tracking-widest uppercase", textColorClass)}>
                        {statusName}
                      </span>
                      <span className="text-[10px] text-on-surface-variant">•</span>
                      <span className="text-[10px] text-on-surface-variant font-label">{formattedTime}</span>
                    </div>
                  </div>
                  <button
                    onClick={handleClose}
              ...
              <div className="flex-1 overflow-y-auto px-5 py-6">
                ...
                {checkpoint.region && (
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <span className="text-[11px] font-label font-bold text-on-surface-variant uppercase tracking-widest">{checkpoint.region}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[9px] font-label font-bold text-on-surface-variant/40 bg-white/5 px-2 py-0.5 rounded uppercase tracking-widest tabular-nums">
                    {updates.length} {lang === 'ar' ? 'تحديث' : 'Updates'}
                  </span>
                </div>
                    ...
                    <div className="space-y-4">
                      {updates.slice(0, 15).map((u, i) => (
                        <motion.div
                          ...
                          className="relative flex gap-4 ms-5"
                        >
